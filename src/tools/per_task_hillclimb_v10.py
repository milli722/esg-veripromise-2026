"""Per-task hillclimb v10: 14-way pool extending v9 with Sprint B p7 focal_g3.

New member (vs v9):
  13 p7_focal_g3 : seed=42, hfl/chinese-macbert-base on combo_v3 hyperparams
                   with focal_gamma raised 2.0 -> 3.0 on T4 (evidence_quality).
                   Single OOF=0.67416 (T1=0.9327 T2=0.4772 T3=0.8640 T4=0.4481),
                   the strongest single-model addition since p4b (0.67355).

Warm-start = v9 winners with 0.0 appended at idx 13 so the search degenerates
back to v9 SOTA (0.68683) when random search fails.

Same biased-search recipe as v7b/v8/v9: every perturbation MUST include the
new member (idx 13). Other 0-2 indices sampled from {0..12}.
"""
...
from src.tools.per_task_hillclimb_v9 import LOADERS as _V9_LOADERS
...
EXPS = [
    ...
    "p7_focal_g3",            # 13   NEW (Sprint B T7)
]

LOADERS = dict(_V9_LOADERS)
LOADERS["p7_focal_g3"] = ("single", "p7_focal_g3", [42])

GRID = [0.0, 0.25, 0.5, 0.75, 1.0, 1.25, 1.5, 1.75, 2.0]
N_RANDOM = 12000
NEW_IDX = [13]                      # p7_focal_g3
LEGACY_IDX = list(range(13))        # 0..12
RNG = random.Random(20260505)

# v9 winners (13-way) with 0.0 appended at idx 13.
SEED_W = {
...
def main() -> None:
    ...
    print()
    print(f"[FINAL v10] per-task weighted score = {ws['final_weighted_score']:.5f}")
    ...
    pd.DataFrame(proc).to_csv(out_dir / "per_task_hillclimb_v10_preds.csv", index=False)
    rows = []
    for t in TASKS:
        ...
        for exp, ww in zip(EXPS, w_tuple):
            row[f"w_{exp}"] = ww
        rows.append(row)
    pd.DataFrame(rows).to_csv(out_dir / "per_task_hillclimb_v10_summary.csv", index=False)
    print(f"[wrote] {out_dir}/per_task_hillclimb_v10_summary.csv")
    print(f"[wrote] {out_dir}/per_task_hillclimb_v10_preds.csv")
