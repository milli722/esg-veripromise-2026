...
- **唯一未測軸：二元 T1/T3 的結構不同模型**，只能靠真實 LB 驗證（現可負擔）。
- 突破 0.61 **仍不確定**——Phase 45 已把二元崩塌歸為「不可約誤差」（可能是 test 標註慣例差異，非閾值問題）。但 `equal9` / `tapt_only` 是**真正未測、近零成本**的 LB 探針，值得用每日上傳預算各打一發。
