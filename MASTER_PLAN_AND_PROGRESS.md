...
> **Phase 47 META 教訓**：(1) **OOF 分布吻合 ≠ 可做 OOF 權重調整**——對抗驗證證明模型「輸出」轉移，但**權重/閾值最優點是擬合到 OOF 雜訊的二階量**，照樣過擬合。永遠用控制組上傳隔離權重調整再信任。(2) 當多個任務各自有已量測的 LB 表現，且後處理對每任務獨立、約束級聯只依賴未變動的任務時，可**用位元級重組組出確定性的新最佳**（c3 的好 T2 + mw_a03 的好 T4），無需再上傳賭運氣。(3) 本賽事可靠槓桿只剩：等權集成、T4 stem 加權（+0.0006）、訊號回收 TTA（待驗）。二元任務的大缺口（T1 −0.155 / T3 −0.186）經 Phase 45 證實為**真實測試準確度差異**（非閾值），post-hoc 修正無解，需重訓更強骨幹方有突破 0.61 的空間。
